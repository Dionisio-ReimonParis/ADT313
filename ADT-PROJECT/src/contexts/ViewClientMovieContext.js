...
export const ViewClientMovieProvider = ({ children }) => {
  ...
  const getUserMovies = async (userId) => {
    try {
      ...
      // Return all movies with proper image paths and related data
      return response.data.map((movie) => ({
        ...
        photos: movie.photos
          ? movie.photos.map((photo) => ({
              ...photo,
              url: photo.url, // Use the direct URL from the database
            }))
          : [],
        videos: movie.videos
          ? movie.videos.map((video) => ({
              ...video,
              url: video.url, // Use the direct URL from the database
            }))
          : [],
      }));
    } catch (error) {
  ...
  const getMovieDetails = async (movieId) => {
    try {
      ...
      // Normalize data structure with proper image paths
      ...
      return {
        ...
        casts: movie.casts
          ? movie.casts.map((cast) => ({
              ...cast,
              url: cast.url, // Use the direct URL from the database
            }))
          : [],
        videos: movie.videos
          ? movie.videos.map((video) => ({
